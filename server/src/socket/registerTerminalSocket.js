import { createPtyProcess } from "../terminal/createPtyProcess.js";
import { findWorkspaceById, getActiveWorkspaceId } from "../workspaces/store.js";

export function registerTerminalSocket({ io, shell, userDir, env }) {
  const activePtys = new Set();

  io.on("connection", (socket) => {
    console.log(`Connected: ${socket.id}`);

    socket.data.workspaceRoot = socket.data.workspaceRoot || userDir;
    socket.data.workspaceId = socket.data.workspaceId || "";

    let ptyProcess = createPtyProcess({ shell, userDir: socket.data.workspaceRoot, env });
